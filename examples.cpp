...
{ {	
	...
	print("\nWhat's the time needed to fly non-stop around the Earth at Mach 1? It's... ");
	auto distance = Earth::equatorial_circumference;
	auto speed = 1_Mach;
	auto time = distance / speed;
	print(time);
} {
	print("\nWhat's the travel time of sun light to Earth? It's... ");
	auto distance = constant::AU;
	auto speed = constant::speed_of_light;
	auto time = distance / speed;
	print(time);
} {
	print("\nWhat's the sum of 1 byte + 1kB + 1GB...(and so on)? It's... ");
	auto sum = 1_byte + 1_kB + 1_MB + 1_GB + 1_TB + 1_PB + 1_EB + 1_ZB + 1_YB + 1_RB + 1_QB;
	print(sum);
} {
	print("\nWhat's the sum of 1m + 1NM + 1ft? It's... ");
	...
	print("\nWhat's the surface area of a soccer ball? It's... ");
	...
	print("\nWhat's the volume of a soccer ball? It's... ");
	auto circumference = 70_cm; // (69-71cm for FIFA ball size 5)
	auto radius = formula::radius_of_circumference(circumference);
	auto volume = formula::volume_of_sphere(radius);
	print(volume);
} {
	print("\nWhat's the surface area of a 30cm pizza? It's... ");
	auto diameter = 30_cm;
	auto area = formula::area_of_circle(diameter / 2);
	print(area);
} {
	print("\nWhat's the volume of a 30cm x 1cm pizza? It's... ");
	auto diameter = 30_cm; 
	auto height = 1_cm;
	auto volume = formula::volume_of_cylinder(diameter / 2, height);
	print(volume);
} {
	print("\nWhat's the distance the Earth has travelled so far? It's... ");
	...
	print("\nWhat's a car's braking distance on dry asphalt from 100km/h? It's... ");
	auto deceleration = 8_m_per_s²; // on dry asphalt
	auto distance = formula::braking_distance(100_km_per_h, 0_km_per_h, deceleration);
	print(distance);
} {
	print("\nWhat's a car's braking distance on wet asphalt from 100km/h? It's... ");
	auto deceleration = 6_m_per_s²; // on wet asphalt
	auto distance = formula::braking_distance(100_km_per_h, 0_km_per_h, deceleration);
	print(distance);
} {
	print("\nHow long takes a 1TB download at 100MBit speed? It's... ");
	auto download_size  = 1_TB;
	auto download_speed = 100_Mbps;
	auto time = download_size / download_speed;
	print(time);
} {
	print("\nWhat's the aircraft's glide path on final at 10NM distance in 3000ft height? It's... ");
	...
	print("\nWhat's the min cable wire size for 100m copper, 230V, 30A max? It's... ");
	...
	print("\nWhat's the impact energy of a 50m asteroid at 50,000km/h? It's... ");
	auto diameter = 50_m;
