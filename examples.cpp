...
{ {	
	...
	print("\n10) What's the time needed to fly non-stop around the Earth at Mach 1? It's... ");
	auto distance = Earth::equatorial_circumference;
	auto speed = 1_Mach;
	auto time = distance / speed;
	print(time);
} {
	print("\n11) What's the travel time of sun light to Earth? It's... ");
	auto distance = constant::AU;
	auto speed = constant::speed_of_light;
	auto time = distance / speed;
	print(time);
} {
	print("\n12) What's the sum of 1 byte + 1kB + 1GB...(and so on)? It's... ");
	auto sum = 1_byte + 1_kB + 1_MB + 1_GB + 1_TB + 1_PB + 1_EB + 1_ZB + 1_YB + 1_RB + 1_QB;
	print(sum);
} {
	print("\n13) What's the sum of 1m + 1NM + 1ft? It's... ");
	auto sum = 1_m + 1_NM + 1_ft;
	print(sum);
} {
	print("\n14) What's the population density on Earth (people per km² of land area)? It's... ");
	auto density = Earth::human_population / Earth::land_area;
	print(density);
} {
	print("\n15) How much land area would be available for each person on Earth? It's... ");
	auto per_person = Earth::land_area / Earth::human_population;
	print(per_person);
} {
	print("\n16) What's the surface area of a soccer ball? It's... ");
	...
	print("\n17) What's the volume of a soccer ball? It's... ");
	auto circumference = 70_cm; // (69-71cm for FIFA ball size 5)
	auto radius = formula::radius_of_circumference(circumference);
	auto volume = formula::volume_of_sphere(radius);
	print(volume);
} {
	print("\n18) What's the surface area of a 30cm pizza? It's... ");
	auto diameter = 30_cm;
	auto area = formula::area_of_circle(diameter / 2);
	print(area);
} {
	print("\n19) What's the volume of a 30cm x 1cm pizza? It's... ");
	auto diameter = 30_cm; 
	auto height = 1_cm;
	auto volume = formula::volume_of_cylinder(diameter / 2, height);
	print(volume);
} {
	print("\n20) What's the distance the Earth has travelled so far? It's... ");
	...
	print("\n21) What's a car's braking distance on dry asphalt from 100km/h? It's... ");
	auto deceleration = 8_m_per_s²; // on dry asphalt
	auto distance = formula::braking_distance(100_km_per_h, 0_km_per_h, deceleration);
	print(distance);
} {
	print("\n22) What's a car's braking distance on wet asphalt from 100km/h? It's... ");
	auto deceleration = 6_m_per_s²; // on wet asphalt
	auto distance = formula::braking_distance(100_km_per_h, 0_km_per_h, deceleration);
	print(distance);
} {
	print("\n23) How long takes a 1TB download at 100MBit speed? It's... ");
	auto download_size  = 1_TB;
	auto download_speed = 100_Mbps;
	auto time = download_size / download_speed;
	print(time);
} {
	print("\n24) What's the aircraft's glide path on final at 10NM distance in 3000ft height? It's... ");
	...
	print("\n27) What's the min cable wire size for 100m copper, 230V, 30A max? It's... ");
	...
	print("\n28) What's the windchill temperature of 5°C air temperature at 55km/h wind? It's... ");
	...
	print("\n29) What's the lift force of an A380 wing on sea level at 284km/h rotation speed? It's... ");
	...
	print("\n30) What's the impact energy of a 50m asteroid at 50,000km/h? It's... ");
	auto diameter = 50_m;
	...
	print("\n31) What's the sound intensity of a 1W loudspeaker at 1m distance? It's... ");
	auto power = 1_W;
	auto distance = 1_m;
	...
	print("\n32) What's the voltage of a capacitor (5V, 0.47µF, 4.7KOhm) after 10ms? It's... ");
	auto CC = 0.47_uF;
	auto V0 = 5_V;
	auto RR = 4.7_kOhm;
	auto time = 10_ms;
	...
	print("\n33) What's the filament length of a 750g PLA roll with 2.85mm diameter? It's... ");
	...
	print("\n34) What's the max diving time in 10m salt water using a 10l bottle? It's... ");
	...
	print("\n35) What's the ballistic max height/range/flight time of a bullet fired 45° on Moon's surface? It's... ");
